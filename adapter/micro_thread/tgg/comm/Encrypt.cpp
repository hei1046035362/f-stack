...
// 显示sha1的值
std::string Encrypt::view_sha1(std::string sha1)
{
    std::string sSha1;
...
std::string Encrypt::Base64Encode(const std::string& input)
{
    // 计算编码后大致需要的缓冲区大小
...
std::string Encrypt::Base64Decode(const std::string& input)
{
...
std::string Encrypt::Aes128Encrypt(const std::string& origData)
{
    if (!Prepared()) {
        return "";
    }
    ...
    return bin2hex(std::string_view(cryptedData.data(), exLen + len), false);
}
...
std::string Encrypt::Aes128Decrypt(const std::string& crypted)
{
    if (!Prepared()) {
        return "";
    }
...
// 模拟defaultEncrypt函数
Encrypt GetEncryptor(const std::string& key, const std::string& iv)
{
...
//     std::string aesdata = "995f0299e4cbd0980229134f96d943a12d672fb28195688cd3b682fab1b6857074339ea89c48ddeeb9aa06288471c425c6add3138d9bf339590f94b2a1e747100b5c1b08688d58da1261b3afde7ccce350eab7f90ab8323a4bf761e4d10a2f6d01fc9a907a384b8eae7579f641942714";
//     std::cout << "__token: " << encryptor.Aes128Decrypt(aesdata) << std::endl;
//     std::cout << "accept key: " << _GenerateAcceptKey("HxrHnnhpu9kW6yZuj1nskw==") << std::endl;    return 0;
// }
...
// __token: {"user_id":46233956298788864,"exp":1731564421,"iat":1730959621,"bot":0}
// accept key: PMOIMdVn9jrSeoeTonRG1usrn+A=
