...
bool RedisClient::Connect(const std::string& redis_password, const std::string& redis_user) {
    char authstr[128] = {0};
...
std::unordered_map<std::string, std::string> RedisClient::ReadMap(const char* key) {
...
std::list<std::string> RedisClient::ReadList(const char* key) {
...
std::set<std::string> RedisClient::ReadSet(const char* key) {
...
std::list<std::string> RedisClient::GetKeys(const char* patterns) {
    std::list<std::string> result;
    for (const auto& info : node_contexts) {
        char cursor[32] = "0";
        do {
...
std::string RedisClient::GetKeyType(const char* key_name)
{
    std::string result;
        // 执行TYPE命令获取键的类型回复
    for (const auto& info : node_contexts) {
...
//     GetUserWithGids(clusterNodes, mapUsers, "bZSCEI3VyV");
